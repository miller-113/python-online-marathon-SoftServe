...
conn = engine.connect()
s = select([books, authors]).where(
    and_(books.c.author_id == authors.c.id_author, books.c.price > 1200))
...
# удление записи
delete_query = books.delete().where(
    books.c.id_book == 1)  # DELETE Books WHERE BOKKS.ID_BOOK == 1;
conn.execute(delete_query)

# обнолвение записи
update_query = books.update().where(books.c.id_book == 2).values(
    title='AnotherTitle')  # UPDATE books SET title= al where books.id_book=3;
...
# соединяет условия WHERE при помощи оператора AND, аналогично конструкции
# select().where()
for row in session.query(Book, Author).filter(
        Book.author_id == Author.id_author).filter(Book.price > 1000):
