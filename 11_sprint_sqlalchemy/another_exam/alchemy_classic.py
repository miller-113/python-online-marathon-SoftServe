...
# Метаданные-это информация о данных в БД; например, информация о таблицах и
# столбцах, в которых мы храним данные.
meta = MetaData()
...
engine = create_engine("mysql+mysqlconnector://root:root@localhost/pyloungedb",
                       echo=True)
...
ins_book_query = books.insert().values(title='Learn Python', author_id=1,
                                       genre='Education', price=1299)
conn.execute(ins_book_query)
ins_book_query2 = books.insert().values(title='Clear Python', author_id=1,
                                        genre='Education', price=956)
conn.execute(ins_book_query2)

books_gr_1000_query = books.select().where(
    books.c.price > 1000)  # SELECT * FROM Books WHERE Books.price > 1000;
...
for row in result:
    print(row)
# 2021-01-27 10:27:59,575 INFO sqlalchemy.engine.base.Engine
# SELECT `Books`.id_book, `Books`.title, `Books`.author_id,
# `Books`.genre, `Books`.price, `Authors`.id_author, `Authors`.name
# FROM `Books`, `Authors`
